...
for j, s := range mod.Sections {
	_=j
	switch section := s.(type) {
	case *wasm.SectionExport:

for i := 0; i < len(section.Entries); i++ {
        fmt.Println("export function field ",section.Entries[i].Field)
}

	case *wasm.SectionCode:
	// can now read function bytecode from section.
	//      fmt.Println(section.Bodies)

	for i := 0; i < len(section.Bodies); i++ {
	fmt.Println("function code ",section.Bodies[i].Code)
	}
	}
}
